"""Clone SQL workspace (sessions, query tasks, field search tasks) between systems. Does not copy query history."""
...
def copy_sql_workspace(source_system_id: int, target_system_id: int) -> None:
    ...
    try:
        src_sessions = (
            db.query(Session)
            .filter(Session.system_id == int(source_system_id))
        ...
        for qt in db.query(QueryTask).filter(QueryTask.session_id.in_(src_ids)).all():
            db.add(
                QueryTask(
                    session_id=id_map[qt.session_id],
                    name=qt.name,
                    sql=qt.sql,
        ...
        for ft in db.query(FieldSearchTask).filter(FieldSearchTask.session_id.in_(src_ids)).all():
            db.add(
                FieldSearchTask(
                    session_id=id_map[ft.session_id],
                    name=ft.name,
